...
# --- NEW API ENDPOINT: Client-side phone number check ---
@csrf_exempt
...
def submit_registration_api(request):
    ...
    This also includes a final check for duplicate phone numbers.
    """
    ...
    try:
        ...
        def safe_decimal(value):
            try: return Decimal(value) if value else None
        ...
        location_data = {}
        if location_str:
            try: location_data = json.loads(location_str)
            except json.JSONDecodeError: logger.warning(f"Failed to decode location JSON: {location_str}")
        ...
        if category == 'individual_labor':
            ...
            instance = Mukkadam(
                ...
                arrange_transport=arrange_transport, arrange_transport_other=arrange_transport_other if arrange_transport == 'other' else None,
        ...
        if location_data.get('latitude') and location_data.get('longitude'):
            try:
                instance.location = Point(
                    float(location_data.get('longitude')), float(location_data.get('latitude')), srid=4326
                )
