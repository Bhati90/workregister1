...
// --- NEW SYNC NOW HANDLER WITH ENHANCED FEEDBACK ---
async function syncNowHandler() {
    const syncButton = document.getElementById('sync-now-btn');
    if (!syncButton) return;
    ...
    if (isOnline) {
        ...
        for (const reg of pendingRegistrations) {
            try {
                // The processAndSendRegistration now returns the error message on failure
                ...
                console.error(`Error submitting pending registration ID ${reg.id}:`, error);
                failedCount++;
                failedMessages.push(error.message || 'An unexpected error occurred.');
            }
        }

        if (syncedCount > 0 && failedCount === 0) {
            alert(`Sync complete! Successfully submitted all ${syncedCount} registration(s).`);
        } else if (syncedCount > 0 && failedCount > 0) {
             alert(`Sync complete with errors. Successfully submitted ${syncedCount} and failed ${failedCount}. Details: ${failedMessages.join('; ')}`);
        } else {
            alert(`Finished sync attempt. Failed to submit ${failedCount} registration(s). Please try again later. Details: ${failedMessages.join('; ')}`);
        }
...
async function handleNextSubmit(event) {
    ...
    if (currentStep === 1) {
        ...
        stepData = {
            full_name: getFieldValue('full_name'), mobile_number: mobileNumber, category: getFieldValue('category'),
            taluka: getFieldValue('taluka'), village: getFieldValue('village'),
            location: (latitude && longitude) ? {
                latitude: parseFloat(latitude), longitude: parseFloat(longitude),
                accuracy: parseFloat(document.getElementById('location_accuracy_hidden').value),
        ...
        if (currentCategory === 'individual_labor') {
            ...
            stepData = { vehicle_type: getFieldValue('vehicle_type'), people_capacity: parseInt(getFieldValue('people_capacity')) || null, expected_fair: getFieldValue('expected_fair'), availability: getFieldValue('availability'), service_areas: getCheckboxValues('service_areas') };
        } else if (currentCategory === 'others') {
            stepData = { business_name: getFieldValue('business_name'), help_description: getFieldValue('help_description') };
        }
        currentRegistration.step2 = stepData;
