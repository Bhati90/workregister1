...
// registration/static/registration/js/serviceworker.js - Updated version with Cloudinary support
...
// Enhanced syncSingleRegistration with Cloudinary support
async function syncSingleRegistration(reg, pendingStore, imageStore) {
    ...
    // Append step 1 data (excluding photo-related fields)
    ...
    // Enhanced image handling for Cloudinary upload
        ...
        try {
            ...
            if (imageData && imageData.image) {
                ...
                if (imageBlob instanceof Blob && imageBlob.size > 0) {
                    // Send image as base64 for reliable offline sync
                    const base64String = await blobToBase64(imageBlob);
                    formData.append('photo_base64', base64String);
                    imageProcessed = true;
                    console.log(`[Service Worker] Image from IndexedDB (ID: ${step1Data.photoId}) converted to base64 for Cloudinary upload. Size: ${imageBlob.size} bytes`);
                }
            }
        } catch (error) {
    ...
    // Priority 2: Fallback to existing base64 if no image was processed from IndexedDB
    if (!imageProcessed && step1Data.photoBase64) {
        try {
            formData.append('photo_base64', step1Data.photoBase64);
            imageProcessed = true;
            console.log(`[Service Worker] Using existing base64 image for Cloudinary upload`);
        } catch (error) {
            console.warn('[Service Worker] Failed to use base64 fallback for sync:', error);
        }
    }
    
    if (!imageProcessed) {
        console.warn('[Service Worker] No image was found or processed for this registration');
    } else {
        console.log('[Service Worker] Image prepared for Cloudinary upload via Django backend');
    }
    ...
    console.log('[Service Worker] --- FormData Contents for Cloudinary Upload ---');
    for (const pair of formData.entries()) {
        if (pair[0] === 'photo_base64') {
            const base64Length = pair[1].length;
            const sizeEstimate = Math.round((base64Length * 3) / 4 / 1024); // Approximate KB
            console.log(`[Service Worker] ${pair[0]}: [Base64 String - ~${sizeEstimate}KB, length: ${base64Length} chars]`);
        } else {
    ...
    // Submit to Django backend - it will handle Cloudinary upload
    ...
    console.log('[Service Worker] Server response:', responseData);
    
    // Check if Cloudinary URL was returned
    if (responseData.photo_url) {
        console.log(`[Service Worker] Image successfully uploaded to Cloudinary: ${responseData.photo_url}`);
    }
    ...
    console.log(`[Service Worker] Registration for ${step1Data.full_name} synced successfully with ID: ${responseData.registration_id}`);
    
    return responseData;
}

// Enhanced syncLaborRegistrations with better success reporting
async function syncLaborRegistrations() {
    console.log('[Service Worker] Attempting to sync offline registrations with Cloudinary support...');
    ...
    try {
        ...
        console.log(`[Service Worker] Found ${registrations.length} pending registrations to sync with Cloudinary upload support.`);
        ...
        for (const reg of registrations) {
            try {
                const syncResult = await syncSingleRegistration(reg, pendingStore, imageStore);
                syncedCount++;
                
                // Track Cloudinary uploads
                if (syncResult && syncResult.photo_url) {
                    cloudinaryUploads++;
                }
        ...
        // Show comprehensive notification with Cloudinary info
        ...
        if (syncedCount > 0) {
            ...
            if (cloudinaryUploads > 0) {
                notificationBody += ` ${cloudinaryUploads} image(s) uploaded to cloud storage.`;
            }
        }
        ...
        console.log(`[Service Worker] Sync complete. Success: ${syncedCount}, Cloudinary Uploads: ${cloudinaryUploads}, Duplicates Removed: ${duplicateCount}, Failed: ${failedCount}`);
...
self.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'SYNC_NOW') {
        ...
        // Manually trigger sync with Cloudinary support
...
self.addEventListener('notificationclick', (event) => {
    ...
    } else if (event.notification.tag === 'duplicate-removed') {
        // Open registration page for new submission
        event.waitUntil(
...
// Add logging for debugging Cloudinary uploads
self.addEventListener('fetch', (event) => {
    // Log Cloudinary-related requests for debugging
    if (event.request.url.includes('cloudinary') || event.request.url.includes('submit-registration')) {
        console.log(`[Service Worker] Request intercepted: ${event.request.method} ${event.request.url}`);
    }
